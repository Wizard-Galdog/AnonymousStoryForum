...
import views.Index;
import views.StoriesPage;
import views.UploadStory;
import views.Login;
import views.Search;
import views.About_Us;
...
public class Main 
{
	...
	{		
		...
		registerShutdownHook(databaseInterface);		System.out.println("Project:csc38");

		Index iIndex = new Index(databaseInterface,fileStoreInterface);

		StoriesPage iStoriesPage = new StoriesPage(databaseInterface,fileStoreInterface);

		UploadStory iUploadStory = new UploadStory(databaseInterface,fileStoreInterface);
		...
		About_Us iAbout_Us = new About_Us(databaseInterface,fileStoreInterface);
			...
			for (int i = 0; i < requestToProcess.size(); i++) 
			{
		        ...
		        //the example dynamic page will process the request if the index.html file is not present
		        	if(toProcess.path.length()==0)
		        		toProcess.path = "index.html";		if(iIndex.process(toProcess))
		{
		}
		else
		if(iStoriesPage.process(toProcess))
		{
		}
		else
		if(iUploadStory.process(toProcess))
		{
		}
		else
		...
		if(iAbout_Us.process(toProcess))
		{
		}
		else
		        {
